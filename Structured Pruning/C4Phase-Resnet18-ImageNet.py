...
class CustomIterativePruner:
    def __init__(self, model, conv_max, fc_max, phase_threshold=0.25, p3_start=0.75, arg1=33, arg2=45, max_out_filt=256, max_mux_trans=8):
        self.model = model
...
def run_utilization_analysis(model):
    ...
    arg1, arg2, arg3, max_mux = 33, 45, 256, 8
...
def run_hybrid_pruning(model, loaders, config):
    ...
    max_mux_trans=config.get('max_mux_trans', 8)
    )
    ...
    while True:
        conv_masks = [m for k, m in pruner.masks.items() if 'conv' in k]
        cnn_s = sum((m == 0).sum().item() for m in conv_masks) / sum(m.numel() for m in conv_masks)
        checkpoint = (copy.deepcopy(model.state_dict()), copy.deepcopy(pruner.masks), copy.deepcopy(pruner.scores))
        ...
        if failed:
            print("\n[!] Accuracy drop too high. Rolling back."); model.load_state_dict(checkpoint[0])
...
if __name__ == "__main__":
    ...
    max_conv_config['conv1'] = 0.6
    max_conv_config['layer1.0.conv1'] = 0.7
    max_conv_config['layer1.0.conv2'] = 0.7
    max_conv_config['layer1.1.conv1'] = 0.7
    max_conv_config['layer1.1.conv2'] = 0.7
    max_conv_config['layer2.0.conv1'] = 0.7
    max_conv_config['layer2.0.conv2'] = 0.8
    max_conv_config['layer2.1.conv1'] = 0.85
    max_conv_config['layer2.1.conv2'] = 0.85

    config = {
        'max_conv': max_conv_config, 'max_fc': {name: 0.0 for name, m in model.named_modules() if isinstance(m, nn.Linear)}, 'step_pct': 0.05,
        'max_drop': 2.0, 'max_drop_p3': 3.0,'p1_threshold': 0.25, 'p3_start': 0.75, 'p3_epochs': 50,  
        'max_epochs': 50, 'top_group_pct': 0.3, 'max_out_filt': 256,
        'max_mux_trans': 8
    }
    ...
    * Pruning Policy: Phase 3 max accuracy drop set to 3% (Evaluating Top-5).
    *max_conv_config['conv1'] = 0.6
    *max_conv_config['layer1.0.conv1'] = 0.7
    *max_conv_config['layer1.0.conv2'] = 0.7
    *max_conv_config['layer1.1.conv1'] = 0.7
    *max_conv_config['layer1.1.conv2'] = 0.7
    *max_conv_config['layer2.0.conv1'] = 0.75
    *max_conv_config['layer2.0.conv2'] = 0.8
    *max_conv_config['layer2.1.conv1'] = 0.85
    *max_conv_config['layer2.1.conv2'] = 0.85
    *config =
